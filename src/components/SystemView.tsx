...
/** Clean schematic: HV source → tank cap Cp ∥ (spark gap → Lp) ⇄ Ls → Cs (topload) → ground. */
    ...
    <div className="w-full h-full grid place-items-center overflow-auto p-6">
      <svg viewBox="0 0 860 420" className="max-w-[920px] w-full" style={{ color: ink }}>
        <g stroke={ink} strokeWidth="1.6" fill="none" strokeLinecap="round">
          ...
          {/* coupling chevrons */}
          <path d="M 452 150 h 0 M 452 150" />
        ...
        <Label x={80} y={262} title="HV supply" value={fmt.si(params.drive.firingVoltage, "V")} />
        ...
        <Label x={672} y={34} title="topload" value={fmt.si(d.CtopPF * 1e-12, "F")} />
        ...
        <g fontFamily="JetBrains Mono" fontSize="11.5" fill={dim}>
          <text x="560" y="300">f₁ = 1/2π√(LpCp) = <tspan fill={ink}>{fmt.si(d.fPrimary, "Hz")}</tspan></text>
          <text x="560" y="320">f₂ = 1/2π√(LsCs) = <tspan fill={ink}>{fmt.si(d.fSecondary, "Hz")}</tspan></text>
          <text x="560" y="340">M = k√(LpLs) = <tspan fill={ink}>{fmt.si(d.M, "H")}</tspan></text>
          <text x="560" y="360" fill={Math.abs(d.fPrimary / d.fSecondary - 1) < 0.05 ? "var(--arc)" : "var(--warn)"}>
